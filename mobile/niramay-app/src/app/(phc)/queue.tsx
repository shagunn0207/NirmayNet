...
export default function PhcQueueScreen() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      <Text style={styles.headerTitle}>{t('phc.queue.title')} ({dummyQueue.length} {t('phc.queue.subtitle')})</Text>
      
      {dummyQueue.map(item => (
        <View key={item.id} style={styles.card}>
          <View style={styles.cardHeader}>
            <View style={styles.headerLeft}>
              <View style={[styles.indicator, { backgroundColor: item.priorityColor }]} />
          ...
          <View style={styles.actionRow}>
            <TouchableOpacity style={styles.primaryButton}>
              <Text style={styles.primaryButtonText}>
                {item.status === 'In Progress' ? t('phc.queue.btn.resume') : t('phc.queue.btn.call')}
              </Text>
            </TouchableOpacity>
